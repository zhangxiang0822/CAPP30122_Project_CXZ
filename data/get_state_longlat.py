...
'''
Note: This Python code file is used to craal the state longitude and latitude
range from the following website:
    - http://www.netstate.com/states/geography/
'''

STATE_ABBR_LIST = ["AL", "AR", "AZ", "CA", "CO", "CT", "DE", "FL", "HI", "IA", \
                   "ID", "IL", "IN", "KY", "MA", "MD", "ME", "MI", "MN", "MO", \
                   "MT", "MS", "NC", "ND", "NE", "NM", "NV", "OH", "OK", "PA", \
                   "RI", "SC", "TN", "TX", "UT", "VA", "VT", "WA", "WI", "WV", \
                   "WY"]

def get_longlatrange(state, state_latlong_range):
    '''
    Crawl the longitude and latitude range of the given state
    
    Inputs:
        - state: (string) Abbreviation of the state
        - state_latlong_range: (dict) dictionary storing the langitude/longitude 
                                      range of the state
                                      
    Return: 
        No return, directly write to the dictionary
...
if __name__ == "__main__":
    ...
    # Hardcode severla states due to lack of data from the source or too
